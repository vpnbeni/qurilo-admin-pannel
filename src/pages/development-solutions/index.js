    ...
    <div className='flex gap-2'>
      <div className='bg-red-100 p-2 rounded-sm'>
        ...
        {data && data.length > 0 ? (
          <ul>
            {data.map(service => (
              <li key={service._id} className='mb-2 p-2 bg-white shadow-lg rounded-sm mt-2 flex justify-between items-end  gap-2'>
                <button className='capitalize font-semibold p-1 px-10' onClick={()=>pagePush.push(`development-solutions/${service.slugName}`)}> {service.servicesName}</button> 
                <button>
...
const MetaupdateModel = ({ service, refreshData }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className='fixed top-0 left-0 h-screen w-full bg-[rgba(0,0,0,0.66)] flex items-center justify-center'>
          <div className='bg-white p-4 translate-x-30 rounded-md h-80 relative  w-1/2   '>
            <button onClick={() => setIsOpen(false)} className='absolute !z-50 -top-7 -right-4 p-1 text-white'>
              <AiOutlineClose size={25} />
            </button>
            <p className='text-base text-black pt-4'>
              <input 
