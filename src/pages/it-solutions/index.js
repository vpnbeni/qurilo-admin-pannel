...
const ItPage = ({ initialData }) => {
  ...
  const handleAddService = async () => {
    const res = await fetch(`${API_URL}auth/v1/it/category`, {
      ...
      body: JSON.stringify({ servicesName: newServiceName }),
    });
...
// // Fetch data on the server side
// export async function getServerSideProps() {
//   const res = await fetch(`${API_URL}auth/v1/service`);
//   const data = await res.json();
//   return {
//     props: {
//       initialData: data.data,
...
//             <ul>
//               {data.map(service => (
//                 <li key={service._id} className='mb-2 p-2 bg-white shadow-lg rounded-sm mt-2 flex justify-between items-end '>
//                   <p className='capitalize font-semibold p-1 px-10'> {service.servicesName}</p>
//                   <button>
//                     <UpdateModel service={service} refreshData={refreshData} />
//                   </button>
//                 </li>
...
//           )}
//         </div>
//         <button className='bg-blue text-white px-4 py-2 font-semibold rounded-md'>ADD</button>
//       </div>
//     </>
//   );
// }
