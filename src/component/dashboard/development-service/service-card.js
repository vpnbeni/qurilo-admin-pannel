...
import DevelopmentSubcategoryCard from "../../cards/DevelopmentSubcategoryCard";
...
export default function ServicesSection({ id }) {
  ...
  const fetchData = async (id) => {
    try {
      ...
      const res = await fetch(
        `${API_URL}auth/v1/development/service-card/${id}`
      );
  ...
  const handleSave = async (idd) => {
    ...
    try {
      const res = await fetch(`${API_URL}auth/v1/development/service-card/`, {
        method: "PUT",
        headers: {
