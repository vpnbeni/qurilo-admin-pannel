...
const EcoDiscuss = ({id}) => {
  ...
  const fetchProjectMind = async () => {
    try {
      const response = await fetch(
        `${API_URL}auth/v1/ecommerce/project-discuss/${id}`
      );
      if (!response.ok) {
  ...
  const handleEditData = async (id) => {
    ...
    // }
    try {
      const response = await fetch(`${API_URL}auth/v1/ecommerce/project-discuss/${id}`, {
        method: 'PUT',
        body: formData,
      });
      if (!response.ok) {
  ...
  return (
    <div>
      <section className='py-20 relative px-6 bg-gradient-to-r from-black to-gray-800 group'>
      ...
      <div className="hidden absolute right-2 bottom-4 group-hover:flex justify-end mt-4 me-5">
          {edit ? (
