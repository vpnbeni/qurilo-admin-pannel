...
export default function EcommerceImplementationCard({
  image,
  title,
  des,
  cardId,
  ...
  const handleSave = async (idd) => {
    ...
    try {
      const response = await fetch(`${API_URL}auth/v1/ecommerce/implementation-card/${idd}`, {
        method: 'PUT',
        body: formData,
      });
      if (response.ok) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`${API_URL}auth/v1/ecommerce/implementation-card/${cardId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        setSuccessfullyEdited(!successfullyEdited);
